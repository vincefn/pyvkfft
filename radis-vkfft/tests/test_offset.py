...
I_arr_FT = np.zeros(Nrev, dtype=np.complex64)

I_arr
#I_arr_FT[-Nrev2:] = np.fft.rfft(I_arr[:Nfwd2])
I_arr_FT[ :Nrev2] = np.fft.rfft(I_arr[:Nfwd2])
...
app.spectrum_d = GPUBuffer(Nfwd*4)
#app.spectrum2_d = GPUBuffer(Nfwd*4)
app.spectrum_FT_d = GPUBuffer(Nrev*8)

app.spectrum_d.setFFTShape(Nfwd2, np.float32)
...
app.command_list = [
    ...
    app.cmdFFT(app.spectrum_d, app.spectrum_FT_d, 0, Nrev2*4),
    app.spectrum_FT_d.cmdTransferStagingBuffer('D2H'),
]
...
app.spectrum_FT_d.toArray(I_arr_FT2)


plt.plot(I_arr_FT.real)
plt.plot(I_arr_FT2.real,'k--')
sys.exit()
# Works!
...
app.command_list = [
    #app.cmdClearBuffer(app.spectrum_FT_d),
    app.spectrum_d.cmdTransferStagingBuffer('H2D'),
    app.cmdFFT(app.spectrum_d, app.spectrum_FT_d, 0, 0),
    app.spectrum_FT_d.cmdTransferStagingBuffer('D2H'),
]
app.writeCommandBuffer()
...
# iteration:
app.spectrum_FT_d.fromArray(I_arr)
app.run()
app.spectrum_FT_d.toArray(I_arr_FT2)
