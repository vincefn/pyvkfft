...
from vulkan_compute_lib import GPUApplication, GPUStruct,  GPUArray
...
def initialize():
    ...
    app = GPUApplication(deviceID=0)

    # Buffers:
    app.init_h = initData()
    app.init_d = GPUStruct.fromStruct(app.init_h, app=app)
    
    app.data_in_d = GPUArray.fromArr(I_arr, binding=2, app=app)
    app.data_FT_d = GPUArray.fromArr(np.zeros((Nb, Nf), np.complex64), app=app)
    app.data_out_d = GPUArray.fromArr(np.zeros((Nb, Nt+2), np.float32), app=app)
