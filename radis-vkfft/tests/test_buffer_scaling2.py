...
##    app.I_arr_d.setData(I_arr)
    ...
    res_ref[:Nb] = np.fft.irfft(I_arr_FT[:Nb])
...
lines_ref = ax.plot(t_arr, np.zeros((Nb_max, Nt), dtype=np.float32).T, '-', lw=1)
lines_gpu = ax.plot(t_arr, np.zeros((Nb_max, Nt), dtype=np.float32).T, '.', lw=1)
